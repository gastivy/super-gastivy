...
export const LastActivity = () => {
  ...
  return (
    <div className="w-full lg:max-w-[40%] max-h-[calc(100dvh-120px)] max-[960px]:h-[calc(100dvh-200px)] flex flex-col gap-2 bg-white rounded-xl p-4 shadow-2xl shadow-zinc-400/10">
        ...
        <Conditional if={!isLoading && activities?.length > 0}>
          <Each
            ...
            render={({ is_done, seconds, ...activity }) => (
              <div
                className="flex flex-col gap-1 py-4 border-b border-zinc-300/20"
                key={activity.id}
              >
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    {is_done ? (
                      <IconFlameFilled size={18} className="text-brand-500" />
                    ) : (
                      <IconClockHour9
                        stroke={2}
                        size={18}

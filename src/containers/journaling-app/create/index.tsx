    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-3">
        <input
          ...
          className="flex-1 rounded-lg border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-950 placeholder:text-zinc-400 outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400 transition-all"
        />
        <input
          type="date"
          value={date}
          onChange={(e) => setDate(e.target.value)}
          className="rounded-lg border border-zinc-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400 transition-all"
        />
      </div>
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs text-zinc-600 hover:text-zinc-900 hover:border-zinc-300 transition-colors"
        >
          <IconBook2 stroke={2} size={14} />
          Templates
        </button>
        {showTemplates && templates.length > 0 && (
          <div className="absolute top-full left-0 mt-1 z-10 w-64 rounded-lg border border-zinc-200 bg-white shadow-lg overflow-hidden">
            {templates.map((t) => (
              <button
                key={t.id}
                onClick={() => handleApplyTemplate(t.content)}
                className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-zinc-50 transition-colors"
              >
                {t.name}
              </button>
            ))}
